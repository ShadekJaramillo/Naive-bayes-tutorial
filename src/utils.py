...
relative_paths = {
    'raw_data' : os.path.join('data', 'raw', 'reviews_data'),
}
...
def csv_to_db_file(csv_data:str):
    reviews_db_path = os.path.join(paths['root_dir'], relative_paths['raw_data']+'.db')
    create_db()
    with sqlite3.connect(reviews_db_path) as connection:
        try:
            df = read_csv(StringIO(csv_data))
            df.to_sql('reviews_data', connection, if_exists='replace')
            print(f'Data successfully written to database at {reviews_db_path}')
...
def save_raw_csv(data:str):
    reviews_csv_path = os.path.join(paths['root_dir'], relative_paths['raw_data']+'.csv')
    with open(reviews_csv_path, 'w', encoding='UTF-8') as f:
        f.write(data)
    print(f'Data successfully saved to csv in {reviews_csv_path}')
...
def create_db():
    ...
    with sqlite3.connect(reviews_db_path) as connection:
        ...
        print(f'Database successfully created at {reviews_db_path}')
